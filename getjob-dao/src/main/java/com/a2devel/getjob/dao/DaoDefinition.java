...
public class DaoDefinition {
	...
	public static void main(String[] args) throws Exception {
		Schema esquema = new Schema(Integer.parseInt(args[0]), args[1]);
		esquema.enableKeepSectionsByDefault();

		addCabeceraPedido(esquema);		
		new DaoGenerator().generateAll(esquema, args[2]);
	 ...
	 * @param esquema
	 */
	private static void defineSchema(Schema esquema) {
		Entity result = esquema.addEntity("Result");
		
		result.setHasKeepSections(true);
		result.addIdProperty();
		result.addStringProperty("url").notNull();
		result.addStringProperty("description").notNull();
		result.addStringProperty("url").notNull();
